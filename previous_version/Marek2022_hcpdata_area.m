...
% Decide whether to adjust the correlation using parenchymal volume as
% covariate using partial correlation analysis
adjust_parenchymal = 'yes';
...
idx_brain_data = startsWith(hcp_data.Properties.VariableNames,'FS_') & ...
    endsWith(hcp_data.Properties.VariableNames,'Area');
...
% Get index of parenchymal volume
idx_parenchymal_volume = ...
    strcmpi(hcp_data.Properties.VariableNames,'FS_BrainSeg_Vol');
...
% Get parenchymal volume data for covariate adjustment of surface area
% measures
parenchymal_volume = ...
    table2array(hcp_data(:,idx_parenchymal_volume));
...
aggregated_table = cat(2,behav_data,brain_data,parenchymal_volume);
...
brain_data_no_missing = aggregated_table_no_missing(:,2:end-1);

% Split parenchymal volume data from brain and behavioral data after
% missing values are removed
parenchymal_volume_no_missing = aggregated_table_no_missing(:,end);

if startsWith(adjust_parenchymal,'y')
    
    % Correlate brain and behavior in the full sample accounting for
    % parenchymal volume
    [r_full, p_full] = ...
        partialcorr(behav_data_no_missing,...
        brain_data_no_missing,...
        parenchymal_volume_no_missing);
...
for i = 1:n_boots
    ...
    for o = 1:n_sample_sizes
        ...
        % Get parenchymal volume after resampling
        parenchymal_volume_no_missing_resampled = ...
            parenchymal_volume_no_missing(...
            resampling_array(1:n_obs_subsample),:);
        ...
        if startsWith(adjust_parenchymal,'y')
            ...
            % parenchymal volume
            [r_res, p_res] = ...
                partialcorr(behav_data_no_missing_resampled,...
                brain_data_no_missing_resampled,...
                parenchymal_volume_no_missing_resampled);
...
% if startsWith(adjust_parenchymal,'y')
%         
%     export_fig('hcp_best_roi_power_area_adjusted.png','-m6','-p0.05')
%     
% else
%     
%     export_fig('hcp_best_roi_power_area_nonadjusted.png','-m6','-p0.05')
%     
% end
...
% if startsWith(adjust_parenchymal,'y')
%        
%    export_fig('hcp_marek_power_area_adjusted.png','-m6','-p0.05')
%    
% else
%    
%    export_fig('hcp_marek_power_area_nonadjusted.png','-m6','-p0.05')
%    
% end
