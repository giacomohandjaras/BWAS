function [type1,type2,typem,types,types_pvals] = abcd_statisticalerrors_ch_mod(Corrs,Pvals,fscorrs,fspvals,iter,thr)
...
% WARNING:
% We commented the line below and opted for providing the function with the
% thresholds for which the type II error rates will be computed, so to
% comply with specific requirements of different procedures (as in our
% simulations).
%thr = [.0000001 .000001 .00001 .0001 .001 .01 .05];
...
for p = 1:length(thr)
    ...
    type1(:,p) = squeeze(nanmean(thisthr,1))';
end
...
pvalthr = [.0000001 .000001 .00001 .0001 .001 .01 .05];
